...
def test_parity_with_simulated_compression(
    ...
    checkpointer.do_save = checkpointer.do_validate = False
    ...
    vllm_model = vllm_runner(
        ...
        max_num_batched_tokens=chunk_size if chunk_size > 0 else None, # TODO test separately
    ...
    checkpointer.override_output = False
    checkpointer.base_dir = './checkpoints'
    checkpointer.tolerance = 1e-4
    checkpointer.set_condition(checkpoint_layer=lambda x: x in CHECK_LAYERS)
    checkpointer.enable_for_prefix("flash_prefix_decode", "flash_prefix_cache", "token_", "flash_attn__")

    tokenizer = AutoTokenizer.from_pretrained(model)
    input_token_ids, reference_token_ids = tokenizer_dependent_random_digit_generator(
        tokenizer, num_digits, random_seed, n_seqs=10, repeat_len=10)
    ...
    vllm_outputs = vllm_model.generate_greedy_logprobs(
        10,
        topk_ll,
        prompt_token_ids=deepcopy(input_token_ids), #[deepcopy(input_token_ids[0]) for _ in range(n_sample)],
        reference_token_ids=deepcopy(reference_token_ids), #[deepcopy(reference_token_ids)[0] for _ in range(n_sample)],
        max_cache_tokens=checkpoint_cfg.max_cache_tokens,
        # target_compression_rate=0.1,
        protected_window_size=checkpoint_cfg.protected_window_size,
        metric_collection_buffer_size=checkpoint_cfg.metric_collection_buffer_size,
...
def test_passing(
    ...
    vllm_outputs = vllm_model.generate_greedy_logprobs(
        10,
        topk_ll,
        prompt_token_ids=[deepcopy(input_token_ids[0]) for _ in range(n_sample)],
        reference_token_ids=[deepcopy(reference_token_ids)[0] for _ in range(n_sample)],
...
def test_compression_with_bias(
    ...
    vllm_model = vllm_runner(
        ...
        protected_window_size=100,
        metric_collection_buffer_size=10,
        kv_head_bias_path=MODEL_KV_HEAD_BIAS["mistralai/Mistral-7B-Instruct-v0.2"],
    )
    max_tokens = max(len(response) for response in random_digit_responses)
    tokenizer = AutoTokenizer.from_pretrained(model)
    reference_token_ids = [
