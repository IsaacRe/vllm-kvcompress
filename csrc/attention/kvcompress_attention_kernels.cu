...
__device__ void single_tier_paged_attention_kernel(
  ...
  const int* __restrict__ kv_metric_buffer_len,  // [num_seqs]
  ...
  const int max_output_metric_position = last_position[seq_idx] - kv_metric_buffer_len[seq_idx];
...
#pragma unroll
  ...
  for (int i = thread_idx; i < num_tokens; i += NUM_THREADS) {
    ...
    logits[i] = normalized;
    if (record_kv_metrics) {
      // Only record attention if outside the metric buffer window
      const int physical_token_idx = physical_block_number * BLOCK_SIZE + physical_block_offset;
      if (kv_position[physical_token_idx] <= max_output_metric_position) {
        kv_metric_out[
          physical_token_idx * num_queries_per_kv + query_head_offset
        ] = normalized;
      }
    }
  }
...
__global__ void single_tier_paged_attention_v1_kernel(
  ...
  const float v_scale,
  const bool record_kv_metrics) {
  single_tier_paged_attention_kernel<scalar_t, cache_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, KV_DTYPE>(
    /* exp_sums */ nullptr, /* max_logits */ nullptr,
    out, kv_metric_out, q, k_cache, v_cache, num_kv_heads, scale, block_tables, context_lens, kv_position, last_position,
    kv_metric_buffer_len, max_num_blocks_per_seq, alibi_slopes, q_stride, kv_block_stride, k_scale, v_scale,
    record_kv_metrics);
}
...
__global__ void single_tier_paged_attention_v2_kernel(
  ...
  const float v_scale,
  const bool record_kv_metrics) {
  single_tier_paged_attention_kernel<scalar_t, cache_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, KV_DTYPE, PARTITION_SIZE>(
    exp_sums, max_logits, tmp_out, tmp_kv_metric_out, q, k_cache, v_cache, num_kv_heads, scale,
    block_tables, context_lens, kv_position, last_position, kv_metric_buffer_len, max_num_blocks_per_seq,
    alibi_slopes, q_stride, kv_block_stride, k_scale, v_scale, record_kv_metrics);
}
...
void kvc_paged_attention_v1_launcher(
  ...
  int* kv_position_ptr = kv_position.data_ptr<int>();
  int* last_position_ptr = last_position.data_ptr<int>();
  int* kv_metric_buffer_len_ptr = kv_metric_buffer_len.data_ptr<int>();
...
#define CALL_KVC_V1_LAUNCHER(T, CACHE_T, BLOCK_SIZE, KV_DTYPE)       \
  kvc_paged_attention_v1_launcher<T, CACHE_T, BLOCK_SIZE, KV_DTYPE>( \
    out,                                                                     \
    kv_metric_out,                                                           \
    query,                                                                   \
    key_cache,                                                               \
...
// 1, 2, 4, 64, 128, 256.
#define CALL_KVC_V1_LAUNCHER_BLOCK_SIZE(T, CACHE_T, KV_DTYPE) \
  switch (block_size) {                                               \
    case 1:                                                                 \
...
void kvcompress_paged_attention_v1(
  ...
  torch::Tensor& kv_metric_buffer_len,  // [num_seqs]
  int64_t block_size,
  ...
  DISPATCH_BY_KV_CACHE_DTYPE(query.dtype(), kv_cache_dtype,
                             CALL_KVC_V1_LAUNCHER_BLOCK_SIZE)
}
...
#define CALL_KVC_V2_LAUNCHER(T, CACHE_T, BLOCK_SIZE, KV_DTYPE)           \
  kvc_paged_attention_v2_launcher<T, CACHE_T, BLOCK_SIZE, KV_DTYPE>(     \
    out,                                                                         \
    kv_metric_out,                                                               \
    exp_sums,                                                                    \
    max_logits,                                                                  \
...
// NOTE(woosuk): To reduce the compilation time, we omitted block sizes
// 1, 2, 4, 64, 128, 256.
#define CALL_KVC_V2_LAUNCHER_BLOCK_SIZE(T, CACHE_T, KV_DTYPE)       \
  switch (block_size) {                                                     \
    case 1:                                                                 \
...
void kvcompress_paged_attention_v2(
  ...
  bool record_kv_metrics) {
  DISPATCH_BY_KV_CACHE_DTYPE(query.dtype(), kv_cache_dtype,
                             CALL_KVC_V2_LAUNCHER_BLOCK_SIZE)
}
