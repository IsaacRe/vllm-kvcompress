...
void set_inputs(
  ...
  set_incr_int_data(
    t2_block_tables, num_seqs * num_blocks_per_seq * num_kv_heads, 1, 1, -1);  
...
int main(int argc, char** argv) {
  ...
  int* t2_block_tables;
  int* context_lens;
  const int num_t2_blocks = num_blocks_per_seq * num_seqs;
  const int num_t2_blocks = num_blocks_per_seq * num_seqs * num_kv_heads;
  const int cache_size = num_t2_blocks * head_size * block_size;
  ...
  cudaMalloc(&t2_block_tables, sizeof(int)* num_t1_blocks * num_kv_heads);
