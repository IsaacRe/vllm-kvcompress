...
class BlockSpaceManagerKVC(BlockSpaceManager):
    ...
    def validate_protected_positions(self, seq: Sequence, cache_moves = None, local_idx = None):
        batch_slot_idx = self.batch_slot_mapping[seq.seq_id]
        last_pos = seq.data.get_len() - 2
        ...
        for i in range(max(0, last_pos - 49), last_pos, 10):
            last_pos_kv = self.kv_metrics.token_positions[mask] == i
            assert last_pos_kv.sum() >= 32 * 32, (i, last_pos)
